        ...
        if (!((activity as MainActivity).isOnline())) {
            ...
            executor.execute {
                ...
                activity?.runOnUiThread {
                    ...
                    movieGenre.text =
                        "Genre: " + genreList.toString().replace("[", "").replace("]", "")
                    movieCompany.text =
                        "Prod. Company: " + companyList.toString().replace("[", "").replace("]", "")
                    movieDirector.text =
                        "Director: " + directorList.toString().replace("[", "").replace("]", "")
                    movieWriter.text =
                        "Writer: " + writerList.toString().replace("[", "").replace("]", "")
                    movieProducer.text =
                        "Producer: " + producerList.toString().replace("[", "").replace("]", "")
                    movieEditor.text =
                        "Editor: " + editorList.toString().replace("[", "").replace("]", "")
                    movieActors.text =
                        "Actors: " + castList.toString().replace("[", "").replace("]", "")
